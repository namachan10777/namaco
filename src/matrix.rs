...
impl Matrix {
    pub fn new<R: Read>(file: &mut R) -> Result<Matrix, MatrixLoadError> {
        ...
        reader
            .read_line(&mut buf)
            .map_err(|_| MatrixLoadError::FailedToReadLine { line: line_cnt })?;
        ...
        let lsize: usize = splited_line[0]
            .parse()
            .map_err(|_| MatrixLoadError::FailedToParseLine { line: line_cnt })?;
        let rsize: usize = splited_line[1]
            .parse()
            .map_err(|_| MatrixLoadError::FailedToParseLine { line: line_cnt })?;
        ...
        loop {
            ...
            let lid: usize = splited_line[0]
                .parse()
                .map_err(|_| MatrixLoadError::FailedToParseLine { line: line_cnt })?;
            let rid: usize = splited_line[1]
                .parse()
                .map_err(|_| MatrixLoadError::FailedToParseLine { line: line_cnt })?;
            let cost: i32 = splited_line[2]
                .parse()
                .map_err(|_| MatrixLoadError::FailedToParseLine { line: line_cnt })?;
